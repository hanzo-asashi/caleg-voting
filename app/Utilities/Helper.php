...
//        }
    ...
    public static function generateNomorSts($id, $bayarId): string
    {
        $bayarId = $bayarId ?? 74;
    ...
    public static function generateNomorStsOp($id, $opid = 1, $delimiter = '/', $length = 5, $pad = '0'): string
    {
    ...
    public static function generateNomorSkpd($id, $bayarId, int $bulan, $tahun, $delimiter = '/', $length = 5, $pad = '0'): string
    {
        ...
        $tahun = $tahun ?: setting('tahun_sppt');
    ...
    public static function generateQrCode($text, $size = 100, $format = 'png', $style = 'square', $eyeStyle = 'square', $margin = 0): string
    {
...
//        return $kode === 0 ? 'Belum Lunas' : 'Lunas';
    ...
    public static function validateDecimal($decimal)
    {
//        $regex = '/^\s*[+\-]?(?:\d+(?:\.\d*)?|\.\d+)\s*$/';
        $regex = '/^\\d+(\\.\\d{1,2})?$/D';
...
//        dd($str);
    ...
    public static function hitungPajakBillboard($nilai, $panjang, $lebar, $format = false)
    {
        ...
        $panjang = static::getFloat($panjang);
        $lebar = static::getFloat($lebar);

        if ($format) {
            return Money::IDR($nilai * ($panjang * $lebar), true);
        }
    ...
    {
        $nilai = $nilai > 0 ? $nilai : 0;
        $kuantiti = static::getFloat($kuantiti);

        if ($format) {
            return Money::IDR($nilai * $kuantiti, true);
        }
